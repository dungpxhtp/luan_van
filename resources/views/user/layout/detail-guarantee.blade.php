 <div class="container">
     <div class="row">
        <div class="col-md-12" style="font-size: 1.6rem;">
            <p class="text-guarantee"> Chế Độ Bảo Hành Tất cả các đồng hồ khi bán ra đều kèm theo 2 phiếu bảo hành:</p>
                <ol>
                  <li> 1 Phiếu Bảo Hành (hoặc Thẻ Bảo Hành/Sổ Bảo Hành) của hãng có giá trị bảo hành Quốc tế (Thời gian bảo hành tùy theo quy định của từng hãng).
                 </li>
                    <li>
                       1 Phiếu Bảo Hành của Cửa Hàng (Sử dụng để được thay pin miễn phí vĩnh viễn & Hưởng chế độ bảo hành tăng thêm từ 1-4 năm của Cửa Hàng).
                    </li>
                   </ol>
             <p class="text-guarantee">
               Ví dụ: Đồng Hồ Citizen có chế độ bảo hành chính hãng: máy = 12 tháng, Pin = 12 tháng.
               </p>
                 <ol>
                   <li>
                      Khi mua tại Cửa Hàng, Khách hàng sẽ được tặng thêm thời gian bảo hành từ 4 năm về máy. Pin = Vĩnh Viễn.
                    </li>
                    <li>
                     Tổng cộng: Khi mua tại Cửa Hàng, đồng hồ Citizen sẽ được bảo hành máy = 05 năm, Pin = Vĩnh Viễn.
                     </li>
                  </ol>
             <p class="text-guarantee">
                Lưu ý:
              </p>
                 <ol>
                  <li>
                     Đối với sản phẩm còn trong thời gian bảo hành Quốc Tế: Quý khách có thể đem tới Cửa Hàng hoặc bất kỳ nhà trung tâm bảo hành nào của hãng được ghi trên phiếu để yêu cầu được kiểm tra đồng hồ.
                   </li>
                    <li>
                       Đối với sản phẩm hết thời gian bảo hành Quốc Tế nhưng vẫn trong thời gian bảo hành tại Cửa Hàng: Quý khách đem đồng hồ kèm Phiếu Bảo Hành của Cửa Hàng tới bất kỳ chi nhánh nào của Cửa Hàng để được hướng dẫn và kiểm tra đồng hồ.
                     </li>
                    </ol>
                    <p class="text-guarantee">
                        Điều Kiện Được Bảo Hành
                     </p>
                        <ol>
                           <li>
                               Bảo hành chỉ có giá trị khi đồng hồ có Phiếu bảo hành của hãng & Phiếu bảo hành của Cửa Hàng đi kèm, điền chính xác, đầy đủ các thông tin.
                              </li>
                            <li>
                               Phiếu bảo hành phải còn nguyên vẹn, không rách, chấp vá, hoen ố, mờ nhạt.
                            </li>
                            <li>
                             Còn trong thời gian bảo hành. Thời gian bảo hành được tính từ ngày mua có ghi trên Phiếu Bảo Hành.
                             </li>
                            <li>
                             Chỉ bảo hành thay thế mới cho những linh kiện, phụ tùng bị hỏng – không thay thế bằng một chiếc đồng hồ khác.
                            </li>
                        </ol>
                                <p class="text-guarantee">
                                     Điều Kiện Không Được Bảo Hành
                                 </p>
                                    <ol>
                                                <li>
                                                    Đồng hồ không có Phiếu bảo hành của hãng và Phiếu bảo hành của Cửa Hàng đi kèm.
                                                </li>
                                                <li>
                                                    Các loại dây đeo, khoá, vỏ, màu xi, mặt số, mặt kiếng bị hỏng hóc, vỡ do sử dụng không đúng, tai nạn, lão hóa tự nhiên, va đập, … trong quá trình sử dụng.
                                                </li>
                                                <li>
                                                    Hỏng hóc do hậu quả gián tiếp của việc sử dụng sai hướng dẫn của hãng có kèm theo đồng hồ.
                                                </li>
                                                <li>
                                                    Trầy xước về dây hoặc mặt kiếng bị trầy xước, vỡ do va chạm trong quá trình sử dụng.
                                                </li>
                                                <li>
                                                    Tự ý thay đổi máy móc bên trong, mở ra can thiệp sửa chữa trong thời gian còn bảo hành – Tại những nơi không phải là trung tâm của hãng
                                                </li>
                                            </ol>
                                    </div>
                                </div>
</div>
